...
const Filter = () => {
    ...
    const renderAccordionItem = (index, title, content, activeIndex, toggleAccordion) => (
        <div className="widget widget-collapsible">
            ...
            <div className={`collapse ${activeIndex === index ? 'show' : ''}`} id={`widget-${index}`}>
    ...
    const renderPriceFilter = (priceRange, setPriceRange) => (
        <div className="filter-price">
            <div className="filter-price-text">
                Price Range:
                <span id="filter-price-range">${priceRange[0]} - ${priceRange[1]}</span>
            </div>
            <Slider
                range
                min={0}
    ...
    const colors = ['#A76D44', '#E6C56E', '#333333', '#D14F41', '#6EA544', '#E88C9D', '#EDEDED', '#4F9CD3'];
    const clearAll = () => {
        setSelectedColor('');
    }
    return (
        <aside className="col-lg-3 order-lg-first">
            <div className="sidebar-product sidebar-shop">
                <div className="widget widget-clean"><label>Filters:</label><a href="#" onClick={() => clearAll()} className="sidebar-filter-clear">Clean All</a></div>

                {renderAccordionItem(0, 'Category', renderCategoryFilters(), activeIndex, toggleAccordion)}
                {renderAccordionItem(1, 'Size', renderSizeFilters(), activeIndex, toggleAccordion)}
                {renderAccordionItem(2, 'Colour', <ColorPalette colors={colors} selectedColor={selectedColor} setSelectedColor={(color) => setSelectedColor(color)} />, activeIndex, toggleAccordion)}
                {renderAccordionItem(3, 'Brand', renderBrandFilters(), activeIndex, toggleAccordion)}
                {renderAccordionItem(4, 'Price', renderPriceFilter(priceRange, setPriceRange), activeIndex, toggleAccordion)}
            </div>
        </aside>
    );
};
