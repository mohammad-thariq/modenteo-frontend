...
const HomePage = () => {
    ...
    const { productsByCategory, similarProducts, productsByCollection, productsByBrand } = new ManageProductsApi();
    ...
    const getProducts = useCallback((type, value) => {
    ...
    }, [productSubcategory, productCategory, productCollection, productBrand, productsData]);
    ...
    return (
        <div>
            {getSettingsByType('banner') && sectionContent?.banner && <WebsiteBanner images={sectionContent.banner} />}
            {renderSection('banner')}
            {getSettingsByType('spotlight') && sectionContent?.spotlight && (
                <div className="pt-5 pb-5">
                    <CategoryCollection data={sectionContent.spotlight} header={getSettingsByType('spotlight')} />
                </div>
            )}
            {renderSection('spotlight')}
            {getSettingsByType('discount') && sectionContent?.discount && (
                <div className="pt-5 pb-5">
                    <Collections data={sectionContent.discount} header={getSettingsByType('discount')} />
                </div>
            )}
            {renderSection('discount')}
            {getSettingsByType('popular') && sectionContent?.popular && (
                <div className="pt-5 pb-5">
                    <Popular data={sectionContent.popular} header={getSettingsByType('popular')} />
                </div>
            )}
            {renderSection('popular')}
            {getSettingsByType('fashion') && sectionContent?.fashion && (
                <div className="pt-5 pb-5">
                    <Fashion images={sectionContent.fashion} header={getSettingsByType('fashion')} />
                </div>
            )}
            {renderSection('fashion')}
            {getSettingsByType('service') && sectionContent?.customerservice && (
                <div className="pt-5 pb-5">
                    <CustomerBenefits data={sectionContent.customerservice} header={getSettingsByType('service')} />
                </div>
            )}
